...
import it.uniroma1.dis.wsngroup.parsing.modules.macro.AbstractModuleMacro;
...
public class StatsModuleMacro extends AbstractModuleMacro {

	public StatsModuleMacro(File fileInput, FileInputStream fis, boolean createVL) {
		...
		if(tsObjectList.size() > 0) {
			...
			while(it.hasNext()) {
				...
				while(edgeIt.hasNext()) {
					...
					EdgeObject edgeObject = new EdgeObject(String.valueOf(edge.getSourceTarget().get(0)), String.valueOf(edge.getSourceTarget().get(1)));
					EdgeObject edgeObjectInv = new EdgeObject(String.valueOf(edge.getSourceTarget().get(1)), String.valueOf(edge.getSourceTarget().get(0)));
			...
			while(it.hasNext()) {
				TimestampObject curTimestampObject = it.next();
				for(Visitor visitor : curTimestampObject.getVisitors()) {
					nodes.add(visitor.getPersonid());
				}
			}
