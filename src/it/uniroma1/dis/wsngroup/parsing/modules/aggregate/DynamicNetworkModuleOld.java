...
import it.uniroma1.dis.wsngroup.parsing.modules.dynamics.AbstractModuleWithoutDBold;
...
@Deprecated
public class DynamicNetworkModuleOld extends AbstractModuleWithoutDBold {
	...
	public DynamicNetworkModuleOld(File fileInput, FileInputStream fis, boolean createVL, Integer idRadix) {
		...
		while(it.hasNext()) {
			...
			if(tags != null) {
				for(int i = 0; i < tags.size(); i++) {
					Integer tagID = Integer.valueOf(tags.get(i).getTagID());
	...
	private class NodeRecord implements Comparable<NodeRecord> {
		...
		public NodeRecord(Integer nodeid, LinkedList<Object> gaps) {
			this.node = nodeid;
			this.gaps = gaps;
		}
