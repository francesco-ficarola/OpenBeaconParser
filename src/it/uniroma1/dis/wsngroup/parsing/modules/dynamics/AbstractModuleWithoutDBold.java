...
@Deprecated
public abstract class AbstractModuleWithoutDBold implements GraphRepresentation {
	...
	private Integer idRadix;
	
	public AbstractModuleWithoutDBold(File fileInput, FileInputStream fis, boolean createVL, Integer idRadix) {
	...
	private void managePacket(ArrayList<String> tokens) {
		...
		if(!currentTimestamp.equals(previousTimestamp)) {
			...
			if(tsObjectList.size() > 0) {
				...
				Integer sourceTag = Integer.parseInt(tokens.get(3).replace("id=", ""), idRadix);
	...
	private void managePrevObject(ArrayList<String> tokens) {
		...
		for(int i = 0; i < tokens.size(); i++) {
			...
			if(tokens.get(i).contains("id=")) {
				...
				int existing = -1;
				Integer idCurrentNode = Integer.parseInt(idSourceNode, idRadix);
			...
			if(tokens.get(0).equals("C") && tokens.get(i).matches("\\x5b\\d+.*")) {
				...
				// Controllo se il tag è già presente nell'oggetto
				int existing = -1;
				Integer idCurrentNode = Integer.parseInt(idTargetNode, idRadix);
				for(int k = 0; k < tsObjectList.get(indexObjects-1).getTags().size(); k++) {
					if(idCurrentNode.equals(Integer.valueOf(tsObjectList.get(indexObjects-1).getTags().get(k).getTagID()))) {
						existing = k;
						break;
				...
				// Controllo se l'arco è già presente nell'oggetto
				boolean existingEdge = false;
				Integer source = Integer.parseInt(idSourceNode, idRadix);
				Integer target = Integer.parseInt(idTargetNode, idRadix);
				...
				if(!existingEdge) {
					ArrayList<Integer> edgeAttr = new ArrayList<Integer>();
					edgeAttr.add(Integer.parseInt(idSourceNode, idRadix));
					edgeAttr.add(Integer.parseInt(idTargetNode, idRadix));
	...
	private void newTimestampObject(ArrayList<String> tokens) {
		...
		for(int i = 0; i < tokens.size(); i++) {
			...
			if(tokens.get(i).contains("id=")) {
				...
				ArrayList<String> readerIDs = new ArrayList<String>();
				readerIDs.add(tokens.get(2).replace("ip=", ""));
				tags.add(new Tag(Integer.parseInt(idSourceNode, idRadix), readerIDs));
			...
			if(tokens.get(0).equals("C") && tokens.get(i).matches("\\x5b\\d+.*")) {
				...
				ArrayList<String> readerIDs = new ArrayList<String>();
				readerIDs.add(tokens.get(2).replace("ip=", ""));
				tags.add(new Tag(Integer.parseInt(idTargetNode, idRadix), readerIDs));
	...
	protected class Tag implements Comparable<Tag> {
		...
		public Tag(int tagID, ArrayList<String> readerIDs) {
			this.tagID = tagID;
